...
export const makeSelectShowChangePassword: TAuthSelectorReturnType<boolean> = createSelector(
  selectState,
  (state?: TAuthStoreRecord): boolean => state?.get('showChangePassword') || false
);
...
export const makeSelectProfileFetching: TAuthSelectorReturnType<boolean> = createSelector(
  selectState,
  (state?: TAuthStoreRecord): boolean => {
    const value = state?.getIn(['profile', 'fetching']);
