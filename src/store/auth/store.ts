...
import {
  ...
  GET_PROFILE_INFO_SUCCESS, SET_SHOW_CHANGE_PASSWORD,
} from './actions';
import {
  ITokenState,
  TAuthStoreRecord,
...
const setShowPassword: TAuthHandler<boolean> = (state, action) =>
  state.set('showChangePassword', action.payload);
...
export default createReducer<TAuthStoreRecord>(initialState, {
  ...
  [SET_SHOW_CHANGE_PASSWORD]: setShowPassword,
});
